...
class DataService {
        ...
        await Promise.all(
            Array.from({ length: config.supportedDevicesNum }, async (_, i) => {
                try {
                    ...
                    console.error(`Error fetching data for device ${i + 1}: ${error.message}`);
                    latestData.push({});
                }
            })
        );
        ...
        await Promise.all(
            Array.from({ length: config.supportedDevicesNum }, async (_, i) => {
                try {
                    ...
                    const latestEntry = await DataModel.find({ deviceId: i, readingDate: { $gte: oneHourAgo } }, { __v: 0, _id: 0 }).sort({ readingDate: 1 });
                    if (latestEntry.length) {
                        latestData.push(latestEntry[0]);
                    }
                } catch (error) {
    ...
    public async getAllBeforeNewest() {
        const latestData: any = [];

        await Promise.all(
            Array.from({ length: config.supportedDevicesNum }, async (_, i) => {
                try {
                    const latestEntry = await DataModel.find({ deviceId: i }, { __v: 0, _id: 0 }).limit(2).sort({ $natural: -1 });

                    if (latestEntry[1]) {
                        latestData.push(latestEntry[1]);
                    }
                } catch (error) {
                    console.error(`Error fetching data for device ${i + 1}: ${error.message}`);
                    latestData.push({});
